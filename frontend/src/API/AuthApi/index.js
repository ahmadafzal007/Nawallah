class AuthController {
    ...
    loginNgo = async(email,password)=>{
        console.log("request received in auth controller");
        console.log("backendURL", `${this.backendURL}/ngo/auth/login`);

        try {
            const response = await fetch(`${this.backendURL}/ngo/auth/login`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok) { // Check if response status is in the range 200-299
                ...
                return (responseData || 'Failed to login NGO'); // Throw an error with a message from the server or a default message
            }
        } catch (error) {
            throw new Error(error || 'Failed to connect to the server'); // Throw an error with a message from the catch block or a default message
        }
